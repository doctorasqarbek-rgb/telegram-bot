...
# Render'da Persistent Disk ulangan bo'lsa, DB_PATH muhit o'zgaruvchisini
# o'sha disk manziliga (masalan "/data/subscribers.db") o'rnating —
# shunda mijozlar ma'lumoti har deploy'da o'chib ketmaydi.
# Agar DB_PATH o'rnatilmagan bo'lsa, joriy papkadagi "subscribers.db"
# ishlatiladi (faqat lokal test uchun mos, Render'da saqlanmaydi!).
# ==========================================================
...
def init_db():
    ...
    conn.commit()
    # Migratsiya: eski bazada "warned" ustuni bo'lmasa qo'shib qo'yamiz
    try:
...
def get_expiring_soon(days=7):
    ...
    # Diapazon bo'yicha tekshiramiz (aynan bitta sanaga emas), shunda job biror kuni
    # ishga tushmay qolsa ham mijoz keyingi kunlarda baribir ogohlantiriladi.
    # "warned=0" orqali bir marta ogohlantirilgandan keyin qayta yubormaymiz.
...
async def log_user_middleware(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Botga kelgan har bir xabardan foydalanuvchini all_users jadvaliga yozib boradi
    (reklama/taklif yuborish uchun to'liq foydalanuvchilar ro'yxatini saqlash maqsadida)."""
...
async def royxat(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """A'zolar o'zlarini bazaga qayta ro'yxatdan o'tkazishlari uchun buyruq
    (masalan, texnik sabab bilan baza tozalangan holatlarda)."""
    user = update.effective_user
    sub = get_subscriber(user.id)
    if sub and sub[5] == 1:
        await update.message.reply_text(
            "Siz allaqachon ro'yxatdasiz.\nObuna muddati: " + str(sub[4]) + " gacha.")
        return
    end_date = add_subscriber(user.id, user.username, user.full_name)
    await update.message.reply_text(
        "✅ Siz muvaffaqiyatli ro'yxatdan o'tkazildingiz!\n"
...
async def reklama_boshlash(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Admin uchun: barcha bot foydalanuvchilariga reklama/taklif xabari yuborish."""
...
async def reklama_tayyorlash(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Admindan kelgan reklama matni/rasmini vaqtincha saqlaydi va tasdiqlashni so'raydi."""
    if update.message.photo:
...
async def reklama_yuborish(context: ContextTypes.DEFAULT_TYPE):
    """Saqlangan reklama xabarini barcha foydalanuvchilarga yuboradi."""
    ...
    for uid, uname, fname in users:
        ...
        await asyncio.sleep(0.05)  # Telegram flood-limit'iga tushmaslik uchun
    ...
    await context.bot.send_message(
        ...
        text="📊 Reklama yuborish yakunlandi!\n\n✅ Yuborildi: " + str(yuborildi) +
             "\n❌ Yuborilmadi (bloklagan/o'chirgan): " + str(yuborilmadi))


async def kirit_boshlash(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Admin uchun: bir nechta a'zoni @username yoki raqamli ID orqali ommaviy qo'shish."""
    if update.effective_user.id != ADMIN_ID:
        return
    ...
    await update.message.reply_text(
        ...
        "@username\n"
        "@username 2026-08-20  (muddatni ham ko'rsatib)\n\n"
        "Agar username bo'lmasa, o'rniga raqamli ID yozing "
        "(ID'ni avvalgi tasdiqlash xabarlaridan \"🆔\" belgisi ostidan topasiz):\n"
        "123456789\n"
        "123456789 2026-08-20\n\n"
        "Masalan:\n"
        "@ali_valiyev\n"
        "123456789 2026-08-15\n\n"
        "Bekor qilish uchun /bekor yozing.")
    ...
    for line in lines:
        ...
        try:
            if identifikator.lstrip("-").isdigit():
                # Raqamli user_id sifatida qidiramiz
                chat = await context.bot.get_chat(int(identifikator))
            else:
                # @username sifatida qidiramiz
                uname_raw = identifikator.lstrip("@")
                chat = await context.bot.get_chat("@" + uname_raw)
...
async def warn_expiring_subscriptions(context: ContextTypes.DEFAULT_TYPE):
    ...
    logging.info(f"warn_expiring_subscriptions ishga tushdi, ogohlantiriladigan mijozlar soni: {len(rows)}")
    for row in rows:
        uid, uname, fname, sd, ed, act, warned = row
        try:
...
async def handle_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.effective_user.id == ADMIN_ID and context.user_data.get("holat") == "bulk_add":
        await bulk_add_process(update, context, text)
        return
